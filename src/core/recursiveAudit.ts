...
import { getAllEscrowTokens, hashIBCDenom, recursiveUnwrapToken } from './ibcUtils.js';
  ...
  try {
    ...
    for (const token of escrowTokens) {
      ...
      if (token.isNative) {
        ...
        ibcCount++;
        const ibcAudit = await auditIbcToken(primaryChainInfo, token, channelId);
...
async function auditNativeToken(
  ...
  try {
    ...
    const secondaryBalance = await fetchTokenBalance(
      secondaryChainInfo,
    ...
    logger.debug(
      `Native token audit - Escrow: ${token.amount}, Secondary: ${secondaryBalance}, Discrepancy: ${discrepancy}`
    );
...
async function auditIbcToken(
  ...
  try {
    ...
    const _escrowBalances = await calculateTotalEscrowBalances(
...
async function calculateTotalEscrowBalances(
  _baseDenom: string,
  _originChain: string,
  _path: Array<{ chain: string; channelId: string; portId: string }>
): Promise<string> {
