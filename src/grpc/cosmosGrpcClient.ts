...
export class CosmosGrpcClient {
  ...
  async getClient(endpoint?: string): Promise<Client> {
    ...
    const clientCredentials =
      this.options.credentials === 'ssl' ? credentials.createSsl() : credentials.createInsecure();
  ...
  private async makeDirectGrpcCall<TRequest, TResponse>(
    ...
    return new Promise((resolve, reject) => {
      ...
      const _metadata = options.metadata || new Metadata();
      const _callOptions: CallOptions = {
